...
abstract class ArrayUtil {
     ...
     * );
     * $values = ArrayUtil::cols($rows, 'value');
     *
     * dump($values);
     *   // 输出结果为
     *   // array(
    ...
    static function getUniqueCols($arr, $col) {
        $ret = array();
        foreach ($arr as $row) {
            if (isset($row[$col])) {
                $ret[] = $row[$col];
     ...
     *     array('id' => 2, 'value' => '2-1'),
     * );
     * $hashmap = ArrayUtil::hashMap($rows, 'id', 'value');
     *
     * dump($hashmap);
     *   // 输出结果为
     *   // array(
     ...
     *     array('id' => 2, 'value' => '2-1'),
     * );
     * $hashmap = ArrayUtil::hashMap($rows, 'id');
     *
     * dump($hashmap);
     *   // 输出结果为
     *   // array(
     ...
     * );
     *
     * $tree = ArrayUtil::tree($rows, 'id', 'parent', 'nodes');
     *
     * dump($tree);
     *   // 输出结果为：
     *   // array(
     ...
     * @code php
     * $refs = null;
     * $tree = ArrayUtil::tree($rows, 'id', 'parent', 'nodes', $refs);
     *
     ...
     * );
     *
     * $rows = ArrayUtil::sortByCol($rows, 'id', SORT_DESC);
     * dump($rows);
     * // 输出结果为：
     * // array(
     * //   array('id' => 6, 'value' => '6-1', 'parent' => 3),
    ...
    static function subArray($array, $subKeys) {
        $subArray = array();
    ...
    static function addField(&$list, $fieldName, $value) {
        foreach ($list as &$v) {
            $v[$fieldName] = $value;
        }
    }
