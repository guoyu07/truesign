...
const src = {
	// css: ['./src/css/**/*.css','!./src/css/lib/*.css'],
	css: './src/css/**/*.css',
...
gulp.task('css',function(){
    cp('./src/css/lib/*.css',dist.css+'lib');
})
...
function online(cb) {
    runSequence('clean','sass', 'css:build','js:build', 'ugjs:build', 'views:build', 'images', 'fonts','dist:online','views:online',function() {
        // 上传静态资源文件到CDN
        cb && cb();
        /*exec('node upload.js', function (err, output) {
         if(err) console.log(err);
...
gulp.task('reload', function () {
    ...
    // runSequence('views','sass','css','js','images','fonts',function () {
