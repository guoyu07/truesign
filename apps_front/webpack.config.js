...
module.exports = {
  ...
  output: {
    app:app,
    path: path.resolve(app_path,publicPath), // html,css,js,图片等资源文件的输出路径，将所有资源文件放在Public目录
    publicPath: publicPath,                  // html,css,js,图片等资源文件的server上的路径
  ...
  plugins: [
    ...
    new webpack.BannerPlugin("Copyright iamsee::ql_os@truesign 真实印记团队 inc.")//在这个数组中new一个就可以了
...
//     filename: prod? app_path+'apps/'+app+'/application/views/' + pathname + '.phtml' : pathname + '.html', // html文件输出路径
