...
class WebSiteController extends  OAppBaseController
{
    ...
    public function regOrLoginAction()
    {
        ...
        $db_checkUsername_response = $doDao->readSpecified(array('username' => $params['username']));
        $db_checkEmail_response = $doDao->readSpecified(array('email' => $params['email']));
        $db_Login_response = $doDao->readSpecified(array('username' => $params['username'], 'pass' => $params['pass']));
