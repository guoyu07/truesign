<template>
    <div id="common_page_model">
        <transition name="el-zoom-in-top" id="common_page_model_body">
            <div v-if="page_data.title" key="haspagedata" class="page_model" :style="page_design" v-loading="authing" :element-loading-text="laoding_text" style="background-color: transparent">
                        ...
                        <div class="page_content" v-if="show_content_footer" :style="page_content_design">
                            <ol>
                                ...
                                <li v-for="(item,index) in page_data.content">
                                    <label class="filelabel" :style=" {borderBottom:page_data.content[index].access?'1px solid rgba(255,255,255,0.41)':'none',
                                    ...
                                    <div v-if="page_data.content[index].type ==='upfile'" class="upfile">
                                        ...
                                        <input class="fileinput" style="display: none" v-model="page_data.content[index].value" :name="page_data.content[index].label" type="text" v-validate.initial="{ rules: { required:(page_data.content[index].regex && page_data.content[index].able_modify )?true:false,regex:page_data.content[index].able_modify?page_data.content[index].regex:false } }" :class="{'input': true, 'is-danger': errors.has(page_data.content[index].label) }" />
                                    </div>
                                    <div v-else-if="page_data.content[index].type ==='upimg'" class="upimg" style="height: 120px;">
                                        <el-upload style="height: 120px" class="avatar-uploader" :show-file-list="false" :drag=false :multiple=false :on-change="AvatarUploadChange" :on-success="handleAvatarSuccess" :on-progress="handleAvatarProgress" :before-upload="beforeAvatarUpload" :data="upload_params.up_param" :action="upload_params.up_action" :http-request="handleAvatarUpload">
                                            <input class="fileinput" style="display: none" v-model="page_data.content[index].value" :name="page_data.content[index].label" type="text" v-validate.initial="{ rules: { required:(page_data.content[index].regex && page_data.content[index].able_modify )?true:false,regex:page_data.content[index].able_modify?page_data.content[index].regex:false } }" :class="{'input': true, 'is-danger': errors.has(page_data.content[index].label) }" />
                                    ...
                                    <div v-else-if="page_data.content[index].type === 'time'" class="timepicker">
                                        <el-date-picker style="display: inline-block;width: 300px" :readonly="!page_data.content[index].able_modify" v-model="page_data.content[index].value" type="datetime" placeholder="选择日期时间" align="left" @change="changetime" :picker-options="pickerOptions">
                                        </el-date-picker>
                                        <input style="display: none" v-model="page_data.content[index].value" :name="page_data.content[index].label" type="text" v-validate.initial="{ rules: { required:(page_data.content[index].regex && page_data.content[index].able_modify )?true:false,regex:page_data.content[index].able_modify?page_data.content[index].regex:false } }" :class="{'input': true, 'is-danger': errors.has(page_data.content[index].label) }" />

                                    </div>
                                    <div v-else-if="page_data.content[index].type === 'text'" style="height: auto;max-height: 300px;overflow-y: auto">
                                    ...
                                    <div v-else-if="page_data.content[index].type === 'radio'" class="widget_radio">
                                        <el-radio-group v-if=" Object.keys(page_data.content[index].widgetType[1]).length <=2" v-model="page_data.content[index].value">
                                            <el-radio v-for="(item,index) in page_data.content[index].widgetType[1]" :key="item" :label='"{\""+index+"\":\""+item+"\"}"'>{{item}}
                                            </el-radio>
                                        </el-radio-group>
                                        <el-select v-else=" Object.keys(page_data.content[index].widgetType[1]).length <=2" v-model="page_data.content[index].value" filterable placeholder="请选择">
                                            <el-option v-for="(item,index) in page_data.content[index].widgetType[1]" :key="index+':'+item" :label="item" :value='"{\""+index+"\":\""+item+"\"}"'>
                                            </el-option>
                                        </el-select>
                                        <input style="display: none" v-model="page_data.content[index].value" :name="page_data.content[index].label" type="text" v-validate.initial="{ rules: { required:(page_data.content[index].regex && page_data.content[index].able_modify )?true:false,regex:page_data.content[index].able_modify?page_data.content[index].regex:false } }" :class="{'input': true, 'is-danger': errors.has(page_data.content[index].label) }" />
                                    </div>
                                    <div v-else-if="page_data.content[index].type === 'checkbox'" class="widget_radio">
                                        <el-checkbox-group v-if=" Object.keys(page_data.content[index].widgetType[1]).length <=2" v-model="page_data.content[index].value">
                                            <el-checkbox v-for="(item,index) in page_data.content[index].widgetType[1]" :key="item" :label='"{\""+index+"\":\""+item+"\"}"'>{{item}}
                                            </el-checkbox>
                                        </el-checkbox-group>
                                        <el-select v-else=" Object.keys(page_data.content[index].widgetType[1]).length <=2" v-model="page_data.content[index].value" filterable multiple placeholder="请选择">
                                            <el-option v-for="(item,index) in page_data.content[index].widgetType[1]" :key="item" :label="item" :value='"{\""+index+"\":\""+item+"\"}"'>
                                            </el-option>
                                        </el-select>
                                    </div>
                                    ...
                                    ">
                                        <div v-if="page_data.content[index].type==='password'">
                                            ...
                                            <input class="" :disabled="!page_data.content[index].able_modify" v-model="page_data.content[index].value" :readonly=" !page_data.content[index].access  || page_data.content[index].key === 'document_id' || !page_data.content[index].able_modify" :style=" {
                                        ...
                                        }" :name="page_data.content[index].label" type="password" v-validate.initial="{ rules: { required:(page_data.content[index].regex && page_data.content[index].able_modify )?true:false,regex:page_data.content[index].able_modify?page_data.content[index].regex:false } }" :class="{'input': true, 'is-danger': errors.has(page_data.content[index].label) }" />

                                        </div>

                                        <input v-if=" page_data.content[index].type !=='password' && page_data.content[index].label === 'id'" class="" :disabled="!page_data.content[index].able_modify" v-model="page_data.content[index].value" :readonly="!page_data.content[index].access  || page_data.content[index].key === 'document_id' || !page_data.content[index].able_modify" :style=" {
                                        borderBottom:page_data.content[index].access?'1px solid rgba(255,255,255,0.41)':'none',
                                        backgroundColor:(page_data.content[index].access && page_data.content[index].able_modify)?'rgba(150, 150, 150, 0.19)':''
                                        }" :name="page_data.content[index].label" type="text" :class="{'input': true, 'is-danger': errors.has(page_data.content[index].label) }" />
                                        <input v-else=" page_data.content[index].type !=='password' && page_data.content[index].label === 'id'" class="" :disabled="!page_data.content[index].able_modify" v-model="page_data.content[index].value" :readonly="!page_data.content[index].access  || page_data.content[index].key === 'document_id' || !page_data.content[index].able_modify" :style=" {
                                        borderBottom:page_data.content[index].access?'1px solid rgba(255,255,255,0.41)':'none',
                                        backgroundColor:(page_data.content[index].access && page_data.content[index].able_modify)?'rgba(150, 150, 150, 0.19)':''
                                        }" :name="page_data.content[index].label" type="text" v-validate.initial="{ rules: { required:(page_data.content[index].regex && page_data.content[index].able_modify )?true:false,regex:page_data.content[index].able_modify?page_data.content[index].regex:false } }" :class="{'input': true, 'is-danger': errors.has(page_data.content[index].label) }" />
                                    </div>
                                    ...
                                    <div style="" class="filediv">
                                        ...
                                        <span style="" v-show="errors.has(page_data.content[index].label)" class="help is-danger filespan">
                                            <i style=" color:rgba(205,52,25,0.38);margin-right: 10px" v-show="errors.has(page_data.content[index].label)" class="fa  fa-refresh rotate_aw"></i>{{ errors.first(page_data.content[index].label) }}
                                        </span>
                                    </div>
        ...
        <transition name="fade-right">
            ...
            <phone_model v-if="page_data.title && show_phone_model" style="display: inline-block;  float: right " :mobile_show_uri="mobile_show_uri"></phone_model>
