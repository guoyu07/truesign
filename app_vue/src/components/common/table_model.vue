    ...
    <div style="overflow: auto">
        ...
        <div id="table_button_bar" style="text-align: left;height: 38px">
            ...
            <div style="display: inline-block">
                <el-input v-focus:currect_select="currect_select" :name="item.search_title" :data-key="item.search_key" style="display: inline-block;width: 180px;" v-for="(item,index) in search_sort_by.search" :key="index" :placeholder="'请输入'+item.search_title" icon="search" v-model="item.search_value">
                </el-input>
            ...
            <el-button v-if="new_add_info" type="primary" style="position: absolute;right: 15px" @click="add_info">{{ new_add_info }}
            </el-button>
        ...
        <transition name="el-zoom-in-top">
            <div class="table_model" key="tabledata" style="" v-if="all_data_count">
                ...
                <el-table v-loading="isloading" element-loading-text="数据加载中" :stripe=false ref="multipleTable" :data="table_data" tooltip-effect="dark" style="width: 100%;overflow: auto" :header-align="'center'" :height="screenHeight-208" :show-summary="true" :sum-text="'汇总'" :border="true" :resizable="true" @summary-method="summaryfunction" :highlight-current-row="true" @selection-change="handleSelectionChange" @row-click="rowClick" @row-dblclick="rowDblClick" @sort-change="sortChange">
                    <el-table-column v-if="groupdelable" fixed type="selection" width="55">
                    </el-table-column>

                    <el-table-column v-for="(item,index) in table_field" :key="index" :show-overflow-tooltip="true" :fixed="index === 'document_id' || index === 'username'" :sortable="is_sortable(index)" :prop="index" :label="item.title" :width="item.width">
                                ...
                                <div v-else="item.widgetStyle.hasOwnProperty(0)" class="nozero">
                                    ...
                                    <div v-for="radio_item,radio_index in get_str2json(scope.row[item.name])">
                                        <div v-if="item.widgetStyle">
                                            <!--如果配置了样式-->
                                            <el-tag v-for="style_item,style_id in item.widgetStyle" v-if="style_id===radio_index" :key="style_id" close-transition :style="style_item"> {{ radio_item }}
                                            </el-tag>
                                ...
                                <div style="display: inline-block;padding-left: 5px" v-for="(item_obj,item_index) in get_str2json(scope.row[item.name])">
                                    <div v-for="type_item,type_id in get_str2json(item_obj)">
                                        ...
                                        <div v-else="item.widgetStyle.hasOwnProperty(0)">
                                            <div v-if="item.widgetStyle">
                                                <!--如果配置了样式-->
                                                <el-tag v-for="style_item,style_index in item.widgetStyle" :key="style_index" v-if="style_index === type_id" close-transition :style="style_item">{{type_item}}
                                                </el-tag>
                                            </div>
                                            <div v-else="item.widgetStyle">
                                                <!--如果没有配置样式-->
                ...
                <div class="table_footer" style="overflow: hidden;margin-top: 2px">
                    ...
                    <el-pagination style="float: right;box-shadow:  0 0 15px black;background-color:#ffffff" :current-page="parseInt(search_sort_by.page)" :page-size="parseInt(search_sort_by.page_size)" @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="[20, 35, 55, 70]" layout="total, sizes, prev, pager, next, jumper" :total="all_data_count">
                    </el-pagination>
                </div>
            </div>
        ...
        <transition name="fade-up" style="">
            <page_model :element-loading-text="loading_text" v-if="wechat_marketing_store.page_model" :final_update_action="info_transfer_action.update" :final_update_btn_desc="'提交修改'" :page_data="detail_page_model_data" source_way="table" :param_apihost="param_apihost" style="position:absolute;z-index:100;
                width:100%;text-align: center;bottom: 10px;height:auto;max-height: 600px;overflow-y: auto;overflow-x: hidden">

            </page_model>

        </transition>
        <phone_model v-if="show_phone_model" style="float: right ;margin-bottom: 100px;position: fixed;top:100px;z-index:2000" :mobile_show_uri="show_phone_model_uri"></phone_model>
    </div>
</template>
