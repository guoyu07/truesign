        ...
        <el-tabs type="border-card" style="background-color: #dcdcdc;box-shadow: none" @tab-click="tabclick"
            ...
            <el-tab-pane v-for="(item,index) in tab_menu_list" :key="item" :label="item.value" :data-name="item.name"
                ...
                <div style="width: 100%;height: auto;min-height: 600px;text-align: left;"
                     v-if="item.name==='客户数据' && defaultTab==='客户数据'" key="客户数据" :tab_name="item.name">
                ...
                <div style="width: 100%;height: auto;min-height: 600px;text-align: left;"
                     v-if="item.name==='级别套餐' && defaultTab==='级别套餐'" key="级别套餐" :tab_name="item.name">
                    <table_model v-loading="isloading" :currect_select="table_currect_select"
                                 :element-loading-text="loading_text" :search_sort_by="table_search_sort_by"
                                 :all_data_count="all_data_count" :table_data="table_model_data"
                                 :table_field="table_model_field"
                                 :new_add_info="'新增套餐'"
                                 :info_transfer_action="info_transfer_action"
                                 groupdelable="true">
                ...
                <div style="width: 100%;height: auto;min-height: 600px;text-align: left;" v-if="item.name==='支付方式'"
                     key="支付方式 && defaultTab==='支付方式'" :tab_name="item.name">
                    <table_model v-loading="isloading"
                                 :currect_select="table_currect_select"
                                 :element-loading-text="loading_text"
                                 :search_sort_by="table_search_sort_by"
