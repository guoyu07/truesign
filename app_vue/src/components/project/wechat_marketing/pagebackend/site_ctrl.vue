    ...
    <div class="top_router_view" style="overflow: scroll">
        <el-tabs type="border-card" style="background-color: #dcdcdc;box-shadow: none" @tab-click="tabclick"
            ...
            <el-tab-pane v-for="(item,index) in tab_menu_list" :key="item" :label="item.value" :data-name="item.name"
                ...
                <div style="width: 100%;height: auto;min-height: 600px;text-align: left;" v-else-if="item.name==='站点设置'"
                     key="站点设置">

                    <page_model :page_data="siteinfo"
                                style="width: 100%;min-width:600px;display: inline-block;vertical-align: top"></page_model>
                </div>
                <div style="width: 100%;height: auto;min-height: 600px;text-align: left;"
                     v-else-if="item.name==='管理员账户管理'" key="管理员账户管理">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getBaseInfo(){
        var vm = this
        this.$http.post(this.report_api + 'getenv', {
          rules: 1,
      ...
      getSiteCfg(){
        var vm = this
        this.$http.post(this.report_api + 'getsitebaseconfig', {
          rules: 1,
          token: this.wechat_marketing_store.token
        }, this.$http_config)
      ...
      getMaster(search_sort_by){
        ...
        this.$http.post(this.report_api + 'getMaster', search_param, this.$http_config)
