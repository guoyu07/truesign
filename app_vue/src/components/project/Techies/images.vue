...
<script>
    ...
    export default {
        ...
        watch: {
            table_search_sort_by: {
                handler: function (val, oldVal) {
                    this.getImages(JSON.stringify(this.table_search_sort_by))
        ...
        created(){
            ...
            this.report_api = this.socket_server_store.apihost + 'Images/'
            this.report_api = 'http://localhost:8089/'
            this.$root.eventHub.$emit('init_navmenu', 'Images')
...
//        search_param.token = this.socket_server_store.token
                this.$http.post(this.report_api + 'Images/getImages?app='+this.server_app, search_param, this.$http_config)
                //        axios.post(this.report_api + 'getImages', search_param, axios_config)
                    .then((res) => {
                        if (res.data.code === 0) {
...
//                                      进行响应式set key
                            ...
                            vm.info_transfer_action = {
                                add: 'Images/DescImages?app='+vm.server_app,
                                get: 'Images/GetImages?app='+vm.server_app,
                                update: 'Images/UpdateImages?app='+vm.server_app,
                                groupdel: 'Images/GroupDelImages?app='+vm.server_app,
                            }
...
//                        vm.isloading = false
            ...
            refresh_table_data(data){
                if (this.defaultTab === '客户数据') {
                    ...
                    this.getImagesLevel(JSON.stringify(this.table_search_sort_by))
                }
