...
<script>
    ...
    export default {
        ...
        created(){
            ...
            this.report_api = this.socket_server_store.apihost + 'User/'
            this.report_api = 'http://localhost:8089/'
            this.$root.eventHub.$emit('init_navmenu', 'User')
...
//        search_param.token = this.socket_server_store.token
                this.$http.post(this.report_api + 'User/getUser?app='+this.server_app, search_param, this.$http_config)
...
//                                      进行响应式set key
                            ...
                            vm.info_transfer_action = {
                                add: 'User/DescUser?app='+vm.server_app,
                                get: 'User/GetUser?app='+vm.server_app,
                                update: 'User/UpdateUser?app='+vm.server_app,
                                groupdel: 'User/GroupDelUser?app='+vm.server_app,
                            }
...
//                        vm.isloading = false
            ...
            refresh_table_data(data){
                if (this.defaultTab === '客户数据') {
                    ...
                    this.getUserLevel(JSON.stringify(this.table_search_sort_by))
                }
