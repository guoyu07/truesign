        ...
        watch: {
            table_search_sort_by: {
                handler: function (val, oldVal) {
                    this.getNote(JSON.stringify(this.table_search_sort_by))
...
//        search_param.token = this.socket_server_store.token
                this.$http.post(this.report_api + 'Note/getNote?app='+this.server_app, search_param, this.$http_config)
...
//                                      进行响应式set key
                            ...
                            vm.info_transfer_action = {
                                add: 'Note/DescNote?app='+vm.server_app,
                                get: 'Note/GetNote?app='+vm.server_app,
                                update: 'Note/UpdateNote?app='+vm.server_app,
                                groupdel: 'Note/GroupDelNote?app='+vm.server_app,
                            }
...
//                        vm.isloading = false
            ...
            refresh_table_data(data){
                if (this.defaultTab === '客户数据') {
                    ...
                    this.getNoteLevel(JSON.stringify(this.table_search_sort_by))
                }
