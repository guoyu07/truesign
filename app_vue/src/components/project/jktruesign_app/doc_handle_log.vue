...
<script>
    ...
    export default {
        ...
        watch: {
            table_search_sort_by: {
                handler: function (val, oldVal) {
                    this.getDochandlelog(JSON.stringify(this.table_search_sort_by))
        ...
        created(){
            ...
            this.report_api = this.socket_server_store.apihost + 'Dochandlelog/'
            this.report_api = 'http://localhost:8089/'
            this.$root.eventHub.$emit('init_navmenu', 'Dochandlelog')
...
//            this.$root.eventHub.$off('currect_row_index')
        ...
        methods: {
            ...
            getDochandlelog(search_sort_by){
                var vm = this
                var search_param = {}
                if (search_sort_by) {
...
//        search_param.token = this.socket_server_store.token
                this.$http.post(this.report_api + 'Dochandlelog/getDochandlelog?app='+this.server_app, search_param, this.$http_config)
                //        axios.post(this.report_api + 'getDochandlelog', search_param, axios_config)
                    .then((res) => {
                        if (res.data.code === 0) {
...
//                                      进行响应式set key
                            ...
                            vm.info_transfer_action = {
                                add: 'Dochandlelog/DescDochandlelog?app='+vm.server_app,
                                get: 'Dochandlelog/GetDochandlelog?app='+vm.server_app,
                                update: 'Dochandlelog/UpdateDochandlelog?app='+vm.server_app,
                                groupdel: 'Dochandlelog/GroupDelDochandlelog?app='+vm.server_app,
                            }
...
//                        vm.isloading = false
            ...
            refresh_table_data(data){
                if (this.defaultTab === '客户数据') {
                    ...
                    this.getDochandlelogLevel(JSON.stringify(this.table_search_sort_by))
                }
