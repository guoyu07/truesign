...
<script>
    ...
    export default {
        ...
        watch: {
            table_search_sort_by: {
                handler: function (val, oldVal) {
                    this.getDoctype(JSON.stringify(this.table_search_sort_by))
        ...
        created(){
            ...
            this.report_api = this.socket_server_store.apihost + 'Doctype/'
            this.report_api = 'http://localhost:8089/'
            this.$root.eventHub.$emit('init_navmenu', 'Doctype')
...
//        search_param.token = this.socket_server_store.token
                this.$http.post(this.report_api + 'Doctype/getDoctype?app='+this.server_app, search_param, this.$http_config)
                //        axios.post(this.report_api + 'getDoctype', search_param, axios_config)
                    .then((res) => {
                        if (res.data.code === 0) {
...
//                                      进行响应式set key
                            ...
                            vm.info_transfer_action = {
                                add: 'Doctype/DescDoctype?app='+vm.server_app,
                                get: 'Doctype/GetDoctype?app='+vm.server_app,
                                update: 'Doctype/UpdateDoctype?app='+vm.server_app,
                                groupdel: 'Doctype/GroupDelDoctype?app='+vm.server_app,
                            }
...
//                        vm.isloading = false
            ...
            refresh_table_data(data){
                if (this.defaultTab === '客户数据') {
                    ...
                    this.getDoctypeLevel(JSON.stringify(this.table_search_sort_by))
                }
