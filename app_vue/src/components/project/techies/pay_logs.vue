...
<script>
    ...
    export default {
        ...
        watch: {
            table_search_sort_by: {
                handler: function (val, oldVal) {
                    this.getPaylogs(JSON.stringify(this.table_search_sort_by))
        ...
        created(){
            ...
            this.report_api = this.socket_server_store.apihost + 'Paylogs/'
            this.report_api = 'http://localhost:8089/'
            this.$root.eventHub.$emit('init_navmenu', 'Paylogs')
...
//        search_param.token = this.socket_server_store.token
                this.$http.post(this.report_api + 'Paylogs/getPaylogs?app='+this.server_app, search_param, this.$http_config)
                //        axios.post(this.report_api + 'getPaylogs', search_param, axios_config)
                    .then((res) => {
                        if (res.data.code === 0) {
...
//                                      进行响应式set key
                            ...
                            vm.info_transfer_action = {
                                add: 'Paylogs/DescPaylogs?app='+vm.server_app,
                                get: 'Paylogs/GetPaylogs?app='+vm.server_app,
                                update: 'Paylogs/UpdatePaylogs?app='+vm.server_app,
                                groupdel: 'Paylogs/GroupDelPaylogs?app='+vm.server_app,
                            }
...
//                        vm.isloading = false
            ...
            refresh_table_data(data){
                if (this.defaultTab === '客户数据') {
                    ...
                    this.getPaylogsLevel(JSON.stringify(this.table_search_sort_by))
                }
