...
class businessAdapter extends DbLibraryAdapter
{
    ...
    {
        return Field::start()
            ->def('document_id')->map('id')->int()->desc('id')->modifiable(false)->issorter('asc')
            ...
            ->def('hide_called_num')->map('hide_called_num')->int()->desc('已拨打隐号通话数量')->modifiable(false)->regex('/^\d{0,6}$/')->issorter('asc')
            ->def('access_sms_msg_num')->map('access_sms_msg_num')->int()->desc('允许发送短信数')->regex('/^\d{0,6}$/')
            ->def('sms_send_num')->map('sms_send_num')->int()->desc('已发送短信数')->modifiable(false)->regex('/^\d{0,6}$/')->issorter(true)
            ->def('enable_time')->map('enable_time')->int()->desc('启用时间')->regex('/^.+$/')->issorter('asc')
            ->def('expire_time')->map('expire_time')->int()->desc('到期时间')->regex('/^.+$/')->issorter('asc')
