...
class siteBaseConfigAdapter extends DbLibraryAdapter
{
    ...
    public function tableInit()
    {
        return Field::start()
            ...
            ->def('site_title')->map('site_title')->varchar(30)->desc('网站标题')->regex('fea')
            ->def('site_keywords')->map('site_keywords')->varchar(30)->desc('网站关键词')
            ->def('site_desc')->map('site_desc')->varchar(100)->desc('网站描述')
            ->def('site_logo_img')->map('site_logo')->varchar(200)->desc('网站logo')
            ->def('site_domain')->map('site_domain')->varchar(120)->desc('网站域名')
            ->def('site_record')->map('site_record')->varchar(120)->desc('网站备案信息')
            ->def('master_phone_num')->map('master_phone')->varchar(120)->desc('站长电话')
            ->def('master_qq_num')->map('master_qq')->varchar(120)->desc('站长qq')
            ->def('master_email')->map('master_email')->varchar(120)->desc('站长EMAIL')
            ->def('custom_service_phone_num')->map('custom_service_phone')->varchar(120)->desc('客服电话')
            ->def('custom_service_qq_num')->map('custom_service_qq')->varchar(120)->desc('客服QQ')
            ...
            ->def('mail_account_mail')->map('mail_account')->varchar(200)->desc('邮箱账户')
            ->def('mail_server')->map('mail_server')->varchar(200)->desc('邮件服务器')
            ->def('mail_server_port_num')->map('mail_server_port')->varchar(200)->desc('邮件服务器端口')
