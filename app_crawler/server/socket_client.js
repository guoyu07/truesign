...
const SOCKET_CLIENT  =  {
    ...
    init : function (unique_auth_code=0,socket_type){
        this.copyright();
        if(!this.data.conn_status){
            this.ws(unique_auth_code,socket_type);
        }


    },

    ws : function(unique_auth_code,socket_type){
        ...
        this.data.wSock  =  new reconnectingwebsocket('ws://127.0.0.1:9501/?unique_auth_code='+unique_auth_code+'&socket_type='+socket_type);
        this.wsOpen();
        this.wsMessage();
        this.wsOnclose();
        this.wsOnerror();
    ...
    wsSend : function(payload){
        ...
        var send_blob
        if(payload){
            send_blob =String2Blob(JSON.stringify(payload))
        }
        else{
            send_blob = String2Blob(JSON.stringify(this.data.payload))
        }
